  ...
  try {
   ...
   if(path==='/api/login'&&method==='POST'){
    ...
    await db.batch([
     ...
     q('INSERT INTO sessions VALUES(?,?,?)',digest(token),user.id,now+12*3600000),
   ...
   if(shareMatch&&['PUT','DELETE'].includes(method)){
    ...
    const owned=await q('SELECT expires FROM shares WHERE token_hash=? AND user_id=? AND expires>?',hash,user.id,now).first();
    ...
    const r=await q('UPDATE shares SET payload=?,expires=? WHERE token_hash=? AND user_id=? AND expires>?',JSON.stringify(safe),p.status==='completed'?Math.min(owned.expires,now+900000):owned.expires,hash,user.id,now).run();
   ...
   const accountMatch=path.match(/^\/api\/admin\/users\/([1-9][0-9]*)\/(password|disable)$/);
   if(accountMatch&&method==='POST'){
    ...
    }else update=q('UPDATE users SET active=0 WHERE id=?',id);
