...
Native builds load `/Advance-Meeter-Taxi-slzzz-oo/`. They require this branch's `assets/native-meter.js`, updated app.js and updated Worker to be deployed there. A compiled wrapper pointing at an older page will not enable native metering. Do not use it for real fares until the device checks pass.
