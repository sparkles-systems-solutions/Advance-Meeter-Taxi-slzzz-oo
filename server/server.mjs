  ...
  try {
   ...
   if(path.startsWith('/api/')){
    ...
    if(shareMatch&&(req.method==='PUT'||req.method==='DELETE')){
     ...
     const safe={id:shareMatch[1],rideId:owned.ride_id,lat:p.lat,lng:p.lng,currentFare:p.currentFare,distanceTraveled:String(p.distanceTraveled).slice(0,20),status:p.status,mode:String(p.mode||'').slice(0,20),timestamp:Date.now(),route};let expires=owned.expires;
