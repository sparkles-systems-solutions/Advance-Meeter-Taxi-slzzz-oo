  ...
  try {
   ...
   if(path.startsWith('/api/')){
    ...
    if(shareMatch&&(req.method==='PUT'||req.method==='DELETE')){
     ...
     const route=p.route&&typeof p.route==='object'?{destination:String(p.route.destination||'').slice(0,300),stops:Array.isArray(p.route.stops)?p.route.stops.slice(0,5).map(x=>String(x).slice(0,300)):[],estimatedRemainingKm:Number.isFinite(p.route.estimatedRemainingKm)?Math.max(0,p.route.estimatedRemainingKm):null,revision:Math.max(0,Math.floor(Number(p.route.revision)||0))}:prior.route;
     const safe={id:shareMatch[1],rideId:owned.ride_id,lat:p.lat,lng:p.lng,currentFare:p.currentFare,distanceTraveled:String(p.distanceTraveled).slice(0,20),status:p.status,mode:String(p.mode||'').slice(0,20),path:safeTrackingPath(p.path),timestamp:Date.now(),route};let expires=owned.expires;
